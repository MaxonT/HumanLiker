(function(){
  ...
  // Improved theme handling with 'system' support
  ...
  function applyTheme(choice){
    ...
    function setScheme(s){ html.setAttribute('data-theme', s); }
    if(!choice || choice==='auto' || choice==='system'){
      setScheme(mq.matches ? 'dark' : 'light');
      try { mq.onchange = (e)=> setScheme(e.matches ? 'dark':'light'); } catch(_){}
    } else { setScheme(choice); }
  }
  const storedTheme = localStorage.getItem(THEME_KEY) || 'auto';
  if(selTheme){
    selTheme.value = storedTheme;
    selTheme.addEventListener('change', e=> setThemeChoice(e.target.value));
  }
  ...
  const i18n = {
  en:{
    nav_dashboard:'Dashboard', nav_demo:'Demo', nav_docs:'Docs', nav_pricing:'Pricing', nav_terms:'Terms', nav_privacy:'Privacy', nav_cookies:'Cookies',
  ...
  es:{
    nav_dashboard:'Panel', nav_demo:'Demo', nav_docs:'Docs', nav_pricing:'Pricing', nav_terms:'Términos', nav_privacy:'Privacidad', nav_cookies:'Cookies',
  ...
  fr:{
    nav_dashboard:'Tableau de bord', nav_demo:'Démo', nav_docs:'Docs', nav_pricing:'Pricing', nav_terms:'Conditions', nav_privacy:'Confidentialité', nav_cookies:'Cookies',
  ...
  pt:{
    nav_dashboard:'Painel', nav_demo:'Demo', nav_docs:'Docs', nav_pricing:'Pricing', nav_terms:'Termos', nav_privacy:'Privacidade', nav_cookies:'Cookies',
  ...
  try{const savedLang=localStorage.getItem('hl_lang')||'en';applyLang(savedLang);if(selLang){selLang.value=savedLang;}}catch(e){}
  ...
  function humanizeText(t, tone, grade){
    ...
    // Formality
    if(grade==='formal'){ t = t.replace(/(can't|won't|don't)/gi, m => ({"can't":"cannot","won't":"will not","don't":"do not"}[m.toLowerCase()])); }
    return t;
  }
...
selLang && selLang.addEventListener('change', e=>{ const v=e.target.value||'en'; try{localStorage.setItem('hl_lang',v);}catch(e){} applyLang(v); });


// --- Modals
const overlay = document.getElementById('modalOverlay');
function openModal(id){ const m=document.getElementById(id); if(!m) return; overlay.hidden=false; m.hidden=false; document.body.style.overflow='hidden'; }
function closeModal(id){ const m=document.getElementById(id); if(!m) return; m.hidden=true; overlay.hidden=true; document.body.style.overflow=''; }
document.querySelectorAll('.open-terms').forEach(a=> a.addEventListener('click', e=>{ e.preventDefault(); openModal('modalTerms'); }));
document.querySelectorAll('.open-privacy').forEach(a=> a.addEventListener('click', e=>{ e.preventDefault(); openModal('modalPrivacy'); }));
document.querySelectorAll('.open-cookies').forEach(a=> a.addEventListener('click', e=>{ e.preventDefault(); openModal('modalCookies'); }));
document.querySelectorAll('.modal-close').forEach(b=> b.addEventListener('click', ()=> closeModal(b.dataset.close)));
overlay && overlay.addEventListener('click', ()=>{
  ['modalTerms','modalPrivacy','modalCookies','modalLogin','modalSignup'].forEach(id=>{ const el=document.getElementById(id); if(el && !el.hidden) closeModal(id); });
});

// --- Auth demo (local)
const openLogin = document.getElementById('openLogin');
const openSignup = document.getElementById('openSignup');
openLogin && openLogin.addEventListener('click', ()=> openModal('modalLogin'));
openSignup && openSignup.addEventListener('click', ()=> openModal('modalSignup'));
function setUser(u){
  const slot=document.querySelector('.controls .auth');
  if(!slot) return;
  if(u){
    slot.innerHTML = `<span class="hint">Hi, ${u.email}</span> <button class="btn ghost" id="logoutBtn">Log out</button>`;
    document.getElementById('logoutBtn').addEventListener('click', ()=>{ localStorage.removeItem('hl_user'); location.reload(); });
  }
}
try{ const cur=JSON.parse(localStorage.getItem('hl_user')||'null'); if(cur) setUser(cur); }catch{}
document.getElementById('loginDo')?.addEventListener('click', ()=>{
  const email=document.getElementById('loginEmail').value.trim();
  const pass=document.getElementById('loginPass').value.trim();
  if(!email || !pass){ document.getElementById('loginHint').textContent='Please fill email and password.'; return; }
  localStorage.setItem('hl_user', JSON.stringify({email})); closeModal('modalLogin'); setUser({email});
});
document.getElementById('signupDo')?.addEventListener('click', ()=>{
  const email=document.getElementById('suEmail').value.trim();
  const pass=document.getElementById('suPass').value.trim();
  const agree=document.getElementById('suAgree').checked;
  if(!agree){ document.getElementById('signupHint').textContent='Please agree to Terms.'; return; }
  if(!email || pass.length<8){ document.getElementById('signupHint').textContent='Check email and password (min 8).'; return; }
  localStorage.setItem('hl_user', JSON.stringify({email})); closeModal('modalSignup'); setUser({email});
});

// --- Subscription demo
document.querySelectorAll('.subscribe').forEach(btn=> btn.addEventListener('click', ()=>{
  const plan=btn.dataset.plan;
  try{ localStorage.setItem('hl_plan', plan); }catch{}
  btn.textContent = `Selected ✓ (${plan})`;
}));

// --- KPI deltas
function deltaTag(isPercent=true){
  const up = Math.random() > 0.2;
  const val = (Math.random()*1.2 + 0.02) * (up?1:-1);
  const span = document.createElement('span');
  span.className = 'kpi-delta ' + (up?'up':'down');
  span.textContent = (up?'+':'') + (Math.abs(val)*100).toFixed(2) + (isPercent?'%':'');
  return span;
}
['kpiTone','kpiHuman','kpiCost','kpiRPM','kpiProgress'].forEach(id=>{
  const box=document.getElementById(id)?.parentElement;
  if(box){ box.appendChild(deltaTag(id!=='kpiCost' && id!=='kpiHuman')); }
});


  // Build language options & apply i18n
  const selLang = document.getElementById('langSelect');
  const LOCALE_KEY = 'hl_locale';
  ...
  function buildLangOptions(){
    if(!selLang) return;
    const codes = ['en','zh','es','fr','de','ko','ja','pt','ar'];
    if(selLang.options.length < 2){
      codes.forEach(code=>{
        const opt = document.createElement('option');
        opt.value = code; opt.textContent = (i18n[code]?.language || code);
  ...
  buildLangOptions();
  const storedLocale = localStorage.getItem(LOCALE_KEY) || 'en';
  if(selLang){
    selLang.value = storedLocale;
    selLang.addEventListener('change', e=> setLocale(e.target.value));
  }
  applyI18n(storedLocale);

  function renderCharts(){
    // Usage line chart
    const usage = document.getElementById('usageChart');
    if(usage){
      const w = usage.clientWidth || 320, h = usage.clientHeight || 180, pad = 28;
      const points = Array.from({length:8}, (_,i)=> (Math.sin(i/1.2)+1)*50 + 20 + Math.random()*10);
      const max = Math.max(...points), min = Math.min(...points);
      const sx = (i)=> pad + i*( (w-2*pad)/(points.length-1) );
      const sy = (v)=> h-pad - (v-min)/(max-min+1e-6) * (h-2*pad);
      const path = points.map((v,i)=> (i?'L':'M') + sx(i).toFixed(1) + ' ' + sy(v).toFixed(1)).join(' ');
      usage.innerHTML = '<defs></defs>' +
        `<line class="axis" x1="${pad}" y1="${h-pad}" x2="${w-pad}" y2="${h-pad}"/>` +
        `<line class="axis" x1="${pad}" y1="${pad}" x2="${pad}" y2="${h-pad}"/>` +
        `<path class="line" d="${path}"/>` +
        points.map((v,i)=> `<circle class="dot" cx="${sx(i).toFixed(1)}" cy="${sy(v).toFixed(1)}" r="3"/>`).join('');
    }
    // Distribution donut
    const donut = document.getElementById('distChart');
    if(donut){
      const w = donut.clientWidth || 320, h = donut.clientHeight || 180;
      const cx=w/2, cy=h/2, r=Math.min(w,h)/3;
      const parts = [0.35,0.25,0.20,0.20];
      let start = -Math.PI/2;
      let svg='';
      const colors = ['var(--primary)','var(--accent)','var(--ok)','var(--warn)'];
      parts.forEach((p,idx)=>{
        const end = start + p*2*Math.PI;
        const x1=cx + r*Math.cos(start), y1=cy + r*Math.sin(start);
        const x2=cx + r*Math.cos(end),   y2=cy + r*Math.sin(end);
        const large = (end-start) > Math.PI ? 1:0;
        svg += `<path class="donut" stroke="${colors[idx%colors.length]}" d="M ${x1} ${y1} A ${r} ${r} 0 ${large} 1 ${x2} ${y2}"/>`;
        start = end;
      });
      donut.innerHTML = svg;
    }
    ...
    if(gauge){
      const w = gauge.clientWidth || 320, h = gauge.clientHeight || 180;
      const cx=w/2, cy=h*0.9, r=Math.min(w,h)*0.42;
      const score = 0.6 + Math.random()*0.35; // 0-1
      const startAng = Math.PI, endAng = 2*Math.PI;
      const arc = (ratio, color)=>{
        const ang = startAng + (endAng-startAng)*ratio;
        const x1=cx + r*Math.cos(startAng), y1=cy + r*Math.sin(startAng);
        const x2=cx + r*Math.cos(ang),      y2=cy + r*Math.sin(ang);
        const large = (ang-startAng) > Math.PI ? 1:0;
        return `<path class="gauge-arc" stroke="${color}" d="M ${x1} ${y1} A ${r} ${r} 0 ${large} 1 ${x2} ${y2}"/>`;
      };
      const needleAng = startAng + (endAng-startAng)*score;
      const nx=cx + (r-6)*Math.cos(needleAng), ny=cy + (r-6)*Math.sin(needleAng);
      gauge.innerHTML = arc(1,'var(--grey-3)') + arc(score,'var(--accent)') + `<line class="needle" x1="${cx}" y1="${cy}" x2="${nx}" y2="${ny}"/>`;
    }
  }
  window.addEventListener('load', renderCharts);

  function updateKPIs(){
    const tone = Math.round(70 + Math.random()*25);
    const human = Math.round(65 + Math.random()*30);
    const cost = (8 + Math.random()*6).toFixed(1);
    const rpm  = Math.round(20 + Math.random()*20);
    const prog = Math.round(60 + Math.random()*40);
    const setNum=(id,v,suffix='')=>{ const el=document.getElementById(id); if(el){ el.textContent=String(v)+suffix; } };
    setNum('kpiTone', tone, '%'); setNum('kpiHuman', human, '%'); setNum('kpiCost', cost); setNum('kpiRPM', rpm); setNum('kpiProgress', prog, '%');
  }
  // Hook into run
  (function(){
    const btn = document.getElementById('runBtn');
    btn && btn.addEventListener('click', ()=>{ updateKPIs(); renderCharts(); });
  })();
