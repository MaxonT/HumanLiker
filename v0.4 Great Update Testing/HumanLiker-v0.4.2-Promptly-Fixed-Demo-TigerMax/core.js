...
document.addEventListener('DOMContentLoaded', function(){
  ...
  (function(){
    ...
    const proceed = document.getElementById('subProceed');
    const subHint = document.getElementById('subHint');
    function close(){ if(modal){ modal.setAttribute('aria-hidden','true'); modal.classList.remove('show'); } }
    closeBtn && closeBtn.addEventListener('click', close);
    modal && modal.addEventListener('click', (e)=>{ if(e.target===modal) close(); });
    proceed && proceed.addEventListener('click', ()=>{ if(subHint){ subHint.textContent='Demo only — no real charge. ✅'; setTimeout(()=>{ subHint.textContent=''; close(); }, 800); } });
    document.querySelectorAll('.subscribe').forEach(b=> b.addEventListener('click', ()=>{
      if(modal){ modal.setAttribute('aria-hidden','false'); modal.classList.add('show'); }
      const plan = b.getAttribute('data-plan')||'free';
      const title = document.getElementById('subTitle'); const desc = document.getElementById('subDesc');
      if(title) title.textContent = 'Subscribe — ' + plan.toUpperCase();
      if(desc)  desc.textContent  = 'Demo flow for ' + plan + ' plan.';
    }));
  })();

  // Humanize + KPIs + charts
  (function(){
    const inputArea = document.getElementById('inputArea');
    const outputArea = document.getElementById('outputArea');
    const runBtn = document.getElementById('runBtn');
    ...
    function updateKPIs(){
      const tone = Math.round(75 + Math.random()*20);
      const human = Math.round(72 + Math.random()*22);
      const cost = (7.5 + Math.random()*4.5).toFixed(1);
      const rpm  = Math.round(18 + Math.random()*24);
    ...
    runBtn && runBtn.addEventListener('click', ()=>{
      const txt = humanizeText(inputArea.value, toneSel?.value||'balanced', gradeSel?.value||'balanced');
      if(outputArea) outputArea.value = txt;
      updateKPIs();
      renderCharts();
    });
    renderCharts();
