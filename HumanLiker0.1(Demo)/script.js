...
const enMarkers = [
  "Honestly, ","In short, ","Big picture: ","Here's the thing, ","To be fair, ","That said, "
]
...
function splitSentencesEN(t){
  const parts = t.replace(/\s+/g,' ').match(/[^.!?]+[.!?]?/g) || [t]
  return parts.map(s=>s.trim()).filter(Boolean)
}
function splitSentencesZH(t){
  const parts = t.replace(/\s+/g,'').split(/(?<=[。！？])/)
  return parts.map(s=>s.trim()).filter(Boolean)
}

function joinShortEN(arr){
  const out=[];let i=0
  while(i<arr.length){
    const a=arr[i]||"",b=arr[i+1]||""
    if(a.length<40 && b.length<40){out.push(a.replace(/[.!?]?$/,'')+", and "+b.toLowerCase());i+=2}
    else{out.push(a);i+=1}
  }
  return out
}
function splitLongEN(arr){
  return arr.flatMap(s=> s.length>140 && s.includes(',')? s.split(',').map(x=>x.trim()).filter(Boolean): [s])
}
function varyEN(sentences, varyP){
  let arr=[...sentences]
  if(chance(varyP)) arr = joinShortEN(arr)
  if(chance(varyP)) arr = splitLongEN(arr)
  return arr
}

function applyPairs(str, pairs){
  let out=str; for(const [rpl, to] of pairs){ out = out.replace(rpl,to) } return out
}
...
function humanizeEN(text, opts){
  ...
  sents = sents.map(s=>{
    let x = s
    if(lvl>0.2) x = applyPairs(x, enSyn)
    if(opts.contractions && lvl>0.3) x = applyPairs(x, enContractions)
    if(opts.markers && chance(0.35*lvl)) x = enMarkers[rand(enMarkers.length)] + x.charAt(0).toLowerCase()+x.slice(1)
    return x
  })
...
function tidyEN(t){
  return t.replace(/\s+,/g, ',').replace(/\s+\./g,'.').replace(/\s+!/g,'!').replace(/\s+\?/g,'?').replace(/\s{2,}/g,' ').trim()
}

function joinShortZH(arr){
  const out=[];let i=0
  while(i<arr.length){
    const a=arr[i]||"",b=arr[i+1]||""
    if(a.length<12 && b.length<12){out.push(a.replace(/[。！？]$/,'')+"，而且"+b);i+=2}
    else{out.push(a);i+=1}
  }
  return out
}
function splitLongZH(arr){
  return arr.flatMap(s=> s.length>40 && s.includes('，')? s.split('，').map(x=>x+"。") : [s])
}
function varyZH(sentences, varyP){
  let arr=[...sentences]
  if(chance(varyP)) arr = joinShortZH(arr)
  if(chance(varyP)) arr = splitLongZH(arr)
  return arr
}
...
function humanizeZH(text, opts){
  ...
  sents = sents.map(s=>{
    let x = s
    if(lvl>0.2) x = applyPairs(x, zhSyn)
    if(opts.markers && chance(0.35*lvl)) x = zhMarkers[rand(zhMarkers.length)] + x
    x = x.replace(/，/g, (m)=> chance(0.15*lvl)? '——' : m)
    return x
  })
...
function humanityScore(text){
  const sentsEN = splitSentencesEN(text).length
  const sentsZH = splitSentencesZH(text).length
  const sents = (sentsEN||0) + (sentsZH||0) || 1
...
function humanize(text, opts){
  ...
  return lang==='zh' ? humanizeZH(text, opts) : humanizeEN(text, opts)
}
